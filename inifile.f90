...
500 error=.true.
  ...
  function Ini_Read_String2(Key, default) result(AValue)
   ...
   character(LEN=Ini_max_string_len) :: AValue
     if (present(default)) then
      AValue = Ini_Read_String_File2(DefIni, Key, default)
     else
      AValue = Ini_Read_String_File2(DefIni, Key)
     end if



  end function Ini_Read_String2
  ...
  function Ini_Read_String_File2(Ini, Key, default) result(AValue)
   Type(TIniFile) :: Ini
  ...
  end function Ini_Read_String_File2
...
10 write (*,*) 'error reading double for key: '//Key
 ...
 function Ini_Key_To_Arraykey_2D(Key, index1,index2)  result(AValue)

    character (LEN=*), intent(IN) :: Key
    integer, intent(in) :: index1, index2
    character(LEN=Ini_max_string_len) :: AValue

    character(LEN=32) :: numstr1, numstr2
    write (numstr1,*) index1
    numstr1=adjustl(numstr1)
    write (numstr2,*) index2
    numstr2=adjustl(numstr2)
    

    AValue = trim(Key) // '(' // trim(numStr1) // ',' // trim(numstr2) // ')' 

 end function Ini_Key_To_Arraykey_2d


!JZ This function reads a 2D array - key(1,2), etc
  function Ini_Read_Double_Array_2D_File(Ini,Key, index1, index2, Default)

  !Reads Key(1), Key(2), etc.

   Type(TIniFile) :: Ini

   double precision Ini_Read_Double_Array_2d_File 
   double precision, optional, intent(IN) :: Default
   integer, intent(in) :: index1, index2
   character (LEN=*), intent(IN) :: Key
   character(LEN=Ini_max_string_len) ::  ArrrayKey

     ArrrayKey = Ini_Key_To_Arraykey_2d(Key,index1,index2)
     if (present(Default)) then
      Ini_Read_Double_Array_2d_File = Ini_Read_Double_File(Ini, ArrrayKey, Default)
     else
      Ini_Read_Double_Array_2d_File = Ini_Read_Double_File(Ini, ArrrayKey)
     end if

  end function Ini_Read_Double_Array_2D_File
